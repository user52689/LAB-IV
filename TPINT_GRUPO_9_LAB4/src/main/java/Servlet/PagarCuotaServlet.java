...
public class PagarCuotaServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (exito) {
                // OPCIÓN 1: Mostrar mensaje de éxito y permanecer en la página de pago
                request.setAttribute("mensajeExito", "Cuota pagada exitosamente.");
                
                // Recargar los datos para mostrar la próxima cuota o el estado actualizado
                Cliente clienteLogueado = (Cliente) session.getAttribute("clienteLogueado");
                int idCliente = clienteLogueado.getIdCliente();
                
                Cuota proximaCuota = cuotaNegocio.obtenerProximaCuotaNoPagada(idPrestamo);
                List<Cuenta> cuentas = cuentaNegocio.listarCuentasPorCliente(idCliente);
                
                request.setAttribute("cuota", proximaCuota);
                request.setAttribute("cuentas", cuentas);
                request.setAttribute("totalCuotas", cuotaNegocio.obtenerTotalCuotas(idPrestamo));
                ...
                // OPCIÓN 2: Si prefieres redirigir con mensaje en sesión (comenta la opción 1 y descomenta esto)
                /*
                session.setAttribute("mensajeExito", "Cuota pagada exitosamente.");
                response.sendRedirect(request.getContextPath() + "/Prestamos/listar");
                */
                
            } else {
                request.setAttribute("mensajeError", "Error al procesar el pago de la cuota.");
                
                // Recargar los datos para mostrar el formulario con el error
                Cliente clienteLogueado = (Cliente) session.getAttribute("clienteLogueado");
                int idCliente = clienteLogueado.getIdCliente();
